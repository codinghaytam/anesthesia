...
def evaluate_policy(agent, n_episodes, ep_len, bis_target_range, bis_fn, features_fn, pk_step_fn, actions):
    rewards = []
...
def plot_training_curves(rewards, losses, save_path="images/training_curves.png", window=50):
...
def plot_bis_trajectory(agent, ep_len, target, bis_target_range, bis_fn, features_fn, pk_step_fn, actions, save_path="images/bis_trajectory.png"):
    c0 = 1.0
    state = np.array([c0, c0 * 0.3, c0 * 0.1, c0 * 0.5], dtype=float)
    ...
    ax1.fill_between(times, bis_target_range[0], bis_target_range[1], alpha=0.2, color="green", label="Target Range")
...
def plot_q_values_heatmap(n_samples, q_max_fn, save_path="images/q_values_heatmap.png"):
    ...
    im = ax.imshow(q_max, extent=[-50, 50, -10, 10], aspect="auto", origin="lower", cmap="RdYlGn", interpolation="bilinear")
...
def plot_policy_heatmap(agent, n_samples, features_fn, actions, save_path="images/policy_heatmap.png"):
    err_range = np.linspace(-50, 50, n_samples)
    derr_range = np.linspace(-10, 10, n_samples)
    ...
    fig, ax = plt.subplots(figsize=(12, 8))
    im = ax.imshow(policy, extent=[-50, 50, -10, 10], aspect="auto", origin="lower", cmap="viridis", interpolation="bilinear")
    ax.set_xlabel("BIS Error", fontsize=12)
    ax.set_ylabel("BIS Error Rate (15s change)", fontsize=12)
...
def plot_action_distribution(agent, n_episodes, ep_len, bis_target_range, bis_fn, features_fn, pk_step_fn, actions, save_path="images/action_distribution.png"):
...
def plot_simple_episode(history_bis, history_actions, target=50.0, save_path=None):
    ...
    ax2.step(range(len(history_actions)), history_actions, color="green", label="Infusion (ml/min)")
    ax2.set_ylabel("Taux d'infusion")
    ax2.set_xlabel("Temps (pas de 5s)")
    ax2.legend()
